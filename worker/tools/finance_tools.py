"""Financial, accounting, valuation, and public-filing tools (adapted for KapexAI).

The SEC helpers that lived in the source project's ``kapexai.tools.runtime``
(``cached_json`` / ``require_env``) do not exist in KapexAI, so they are
reimplemented here. ``cached_json`` uses an in-process TTL cache with ``httpx``;
``require_env`` reads the environment and raises a friendly ``FinanceToolError``.

These tools are internal to the single ``finance`` tool: they are bound to the
tool's internal LangChain react agent and are NEVER registered in KapexAI's main
tool registry (``worker/tools/registry.py``).
"""
...
class FinanceToolError(Exception):
    """A user-facing finance tool error (missing config, failed request, etc.)."""
...
def require_env(name: str) -> str:
    ...
    if not value:
        raise FinanceToolError(
            f"SEC data is unavailable: the {name} environment variable is not set. "
            "Ask the administrator to configure it with an application name and contact email."
        )
    return value
...
def cached_json(
    ...
    """GETs a URL and returns parsed JSON, cached in-process for `ttl_seconds`.

    Raises ``FinanceToolError`` with a friendly message on any failure so the
    calling tool can surface it to the user instead of leaking an httpx error.
    """
    key = f"{method} {url}"
    now = time.monotonic()
    ...
    try:
        ...
        raise FinanceToolError(
            f"SEC request failed ({url}): {exc}. Please try again shortly."
        ) from exc
    except ValueError as exc:  # JSON decode failure
        raise FinanceToolError(
            f"SEC returned an unexpected response ({url}). Please try again shortly."
        ) from exc
...
def _normalize_cik(cik: str | int) -> str:
    ...
    if not digits or len(digits) > 10:
        raise FinanceToolError("CIK must contain between 1 and 10 digits")
...
def sec_company_lookup(ticker: str) -> dict:
    ...
    try:
        ...
        return {"ticker": ticker, "cik": None, "name": None, "error": str(exc)}
...
def sec_company_submissions(
    ...
    try:
        ...
        return {"cik": cik, "filings": [], "error": str(exc)}
...
def sec_company_facts(
    ...
    try:
        ...
        return {"cik": cik, "facts": {}, "error": str(exc)}
