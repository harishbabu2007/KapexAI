"""Deterministic equity-investment calculator tools (adapted for KapexAI).

These are internal to the single ``finance`` tool: they are bound to the tool's
internal LangChain react agent and are NEVER registered in KapexAI's main tool
registry (``worker/tools/registry.py``).
"""
...
from worker.tools.finance_calculators import (
    _future_value,
    _nonnegative,
    _positive,
)
